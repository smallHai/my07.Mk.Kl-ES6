...
/***/ (function(module, exports, __webpack_require__) {
	...
	var syy = new _lottery2.default(); //-兼容处理模块，垫片
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } //整合成完整彩票模块
	...
	var Lottery = function (_mix) {
	    ...
	    function Lottery() {
	        ...
	        _this.name = name;
	        _this.cname = cname;
	        _this.issue = issue; //期号
	        _this.state = state; //状态
	        _this.el = ""; //当前节点
	        _this.omit = new Map(); //遗落数据
	        _this.open_code = new Set(); //开奖号码
	        _this.open_code_list = new Set(); //开奖记录
	        _this.play_list = new Map(); //玩法列表
	        _this.number = new Set(); //选号
	        _this.issue_el = "#curr_issue"; //期号节点
	        _this.countdown_el = "#countdown"; //倒计时节点
	        _this.state_el = ".state_el"; //状态节点
	        _this.cart_el = ".codelist"; //号码列表节点
	        _this.omit_el = ""; //遗落节点
	    ...
	    _createClass(Lottery, [{
	        ...
	        value: function initEvent() {
	            var self = this;
	            //bind主要为了把this指向当前这个self
	            (0, _jquery2.default)("#plays").on("click", "li", self.changePlayNav.bind(self)); //玩法切换
	            (0, _jquery2.default)(".boll-list").on("click", ".btn-boll", self.toggleCodeActive.bind(self)); //号码选择
	            (0, _jquery2.default)("#confirm_sel_code").on("click", self.addCode.bind(self)); //确认选号
	            (0, _jquery2.default)(".dxjo").on("click", "li", self.assistHandle.bind(self)); //操作区
	...
	var Base = function () {
	    ...
	    _createClass(Base, [{
	        ...
	        value: function initNumber() {
	            for (var i = 1; i < 12; i++) {
	                this.number.add(('' + i).padStart(2, "0")); //这里用的set数据结构，不满足2位前面填充0
	            }
	        }
	        ...
	        value: function toggleCodeActive(e) {
	            var self = this;
	            var $cur = (0, _jquery2.default)(e.currentTarget); //获取当前选到的节点
	        ...
	        value: function addCode() {
	            ...
	            var count = self.computerCount(active, self.cur_play);
	            if (count) {
	        ...
	        value: function getCount() {
	            ...
	            var count = self.computerCount(active, self.cur_play);
	            var range = self.computerBonus(active, self.cur_play);
	            var money = count * 2;
	        ...
	        value: function getRandom(num) {
	            var arr = [];
	            var index = void 0;
	            var number = Array.from(this.number); //把一个数据集合转成数组
	            while (num--) {
...
/***/ (function(module, exports) {
	...
	var Calculate = function () {
	    ...
	    _createClass(Calculate, [{
	        ...
	        //计算注数
	        value: function computerCount(active, play_name) {
	            var count = 0;
	            var exist = this.play_list.has(play_name); //play_list使用了map类型
	            var arr = new Array(active).fill('0'); //用0填充初始化为已知长度active的数组
	        ...
	        //计算奖金金额
	        value: function computerBonus(active, play_name) {
	        ...
	        value: function combine(arr, size) {
	            ...
	            (function f(arr, size, result) {
	                ...
	                if (size === arrLen) {
	                    allResult.push([].concat(result, arr)); //把最后的一个数组和result保存在最后结果中
	                } else {
	...
	var Interface = function () {
	    ...
	    _createClass(Interface, [{
	        ...
	        value: function getOmit(issue) {
	            var self = this; //这边先保留this指向，防止下面箭头函数改变指向
	            return new Promise(function (resolve, reject) {
	                //用promise代替传统的回调函数
	                _jquery2.default.ajax({
	                    ...
	                    success: function success(res) {
	                        self.setOmit(res.data); //继承了其它类的方法,用来保存数据，可以达到数据共享
	    ...
	    _createClass(Timer, [{
	        ...
	        value: function countdown(end, update, handle) {
	            ...
	            if (now - end > 0) {
	                ...
	                self.last_time = r.join(''); //将数组保存到类的属性中
	                update.call(self, r.join('')); //每判断完一次就更新一次
	                setTimeout(function () {
	                    //循环执行
